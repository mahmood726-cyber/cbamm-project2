...
robust_rma <- function(yi, sei, data = NULL, method = "REML",
  ...
  if (inherits(fit, "try-error")) {
    ...
    fit <- try(do.call(metafor::rma, args2), silent = TRUE)
    if (inherits(fit, "try-error") && use_hksj) {
      args2$test <- NULL
      fit <- try(do.call(metafor::rma, args2), silent = TRUE)
    }
  }
...
run_core_meta_analysis <- function(data, config) {
  ...
  if (config$methods$robust_variance && any(duplicated(data$study_id))) {
...
run_transport_analysis <- function(data, target_population, config) {
  ...
  if (config$methods$grade_weighting && "grade" %in% names(data)) {
