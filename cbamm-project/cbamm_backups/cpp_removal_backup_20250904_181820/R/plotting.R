...
.create_funnel_plot <- function(meta_results, config) {
  ...
  p <- ggplot2::ggplot(plot_data, ggplot2::aes(x = yi, y = sei)) +
    ...
    ggplot2::labs(x = "Effect Size", y = "Standard Error", 
                  title = "Funnel Plot") +
    ggplot2::theme_minimal()
...
#' @keywords internal
.create_multiverse_plot <- function(multiverse, config) {
  # Placeholder for multiverse plot
  p <- ggplot2::ggplot() + 
    ggplot2::labs(title = "Multiverse Plot", subtitle = "Implementation in progress") +
    ggplot2::theme_minimal()
