...
run_publication_bias_sensitivity <- function(data) {
  ...
  if (check_package("weightr", quietly = TRUE)) {
    ...
    sm <- try(pure_r_selection_model(effect = data$yi, v = data$se^2, steps = br, table = FALSE), silent = TRUE)
...
#' Build pure R selection modeling Breaks
#' @keywords internal
...
rve_print <- function(res, cluster_vec, label = "[RVE-CR2]") {
  ...
  vc <- try(pure_r_robust_vcov(res, cluster = cluster_vec, type = "CR2"), silent = TRUE)
  if (inherits(vc, "try-error")) return(invisible(NULL))
  
  rob <- try(pure_r_coef_test(res, vcov = vc), silent = TRUE)
