...
cbamm <- function(data, config = cbamm_config(), target_population = NULL, 
  ...
  if (nrow(data) > 5000) {
    if (config$output$verbose) {
      message("Large dataset detected (n=", nrow(data), "). Applying systematic thinning to 5,000 studies for stability.")
    }
    # Systematic thinning to preserve distribution
    thin_idx <- round(seq(1, nrow(data), length.out = 5000))
    data <- data[thin_idx, ]
    preinit_warnings <- c(
      preinit_warnings,
      "Memory: Dataset thinned to 5,000 studies for computational stability."
    )
  }
  ...
  results$diagnostics$warnings <- c(results$diagnostics$warnings, preinit_warnings)
  ...
  if (config$methods$transport) {
    ...
    tryCatch({
      ...
      if (!is.null(results$transport_results) && "transport_weights" %in% names(results$transport_results)) {
        ...
        results$meta_results$transport <- results$transport_results$transport
      }
      if (!is.null(results$transport_results) && "analysis_weights_grade" %in% names(results$transport_results)) {
        data$analysis_weights_grade <- results$transport_results$analysis_weights_grade
      }
      if (!is.null(results$transport_results) && "grade" %in% names(results$transport_results)) {
        results$meta_results$grade <- results$transport_results$grade
      }
    }, error = function(e) {
  ...
  if (isTRUE(config$methods$cross_design) && "study_type" %in% names(data)) {
    if (config$output$verbose) message("Running cross-design synthesis...")
    tryCatch({
      study_type_norm <- tolower(as.character(data$study_type))
      rct_idx <- study_type_norm %in% c("rct", "trial", "randomized")
      obs_idx <- study_type_norm %in% c("obs", "observational", "cohort", "case_control", "cross_sectional")
      design_weights <- if (isTRUE(config$methods$grade_weighting) && "analysis_weights_grade" %in% names(data)) {
        data$analysis_weights_grade
      } else if ("analysis_weights" %in% names(data)) {
        data$analysis_weights
      } else {
        NULL
      }

      if (sum(rct_idx, na.rm = TRUE) > 0 && sum(obs_idx, na.rm = TRUE) > 0) {
        effective_variance <- compute_effective_variance(data$se, design_weights)
        rct_data <- data.frame(
          effect = data$yi[rct_idx],
          variance = effective_variance[rct_idx]
        )
        obs_data <- data.frame(
          effect = data$yi[obs_idx],
          variance = effective_variance[obs_idx]
        )

        results$cross_design_results <- cross_design_synthesis(
          rct_data = rct_data,
          obs_data = obs_data
        )
        optimizations <- c(optimizations, "cross_design_synthesis")
      }
    }, error = function(e) {
      warning("Cross-design synthesis failed: ", e$message)
      results$diagnostics$warnings <- c(results$diagnostics$warnings, paste("Cross-design:", e$message))
    })
  }
  ...
  if (isTRUE(config$methods$multiverse)) {
    ...
    tryCatch({
      ...
      results$diagnostics$warnings <- c(results$diagnostics$warnings, paste("Multiverse:", e$message))
    })
  }
...
validate_cbamm_data <- function(data) {
  ...
  if ("study_type" %in% names(data)) {
    ...
    invalid_types <- setdiff(as.character(unique(data$study_type)), valid_types)
