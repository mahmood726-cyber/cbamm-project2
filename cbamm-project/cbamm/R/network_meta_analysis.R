...
#' @param model Model type: "random" (default), "fixed", or "component"
...
network_meta_analysis <- function(data, 
  ...
  # Input validation
  validate_network_inputs(data, studies, treatments, effect_size, std_error, model)
  ...
  if (model == "component") {
    nma_results <- run_component_nma(network_data, reference)
  ...
  if (inconsistency && model != "component") {
...
validate_network_inputs <- function(data, studies, treatments, effect_size, std_error, model = "random") {
  ...
  validate_network_structure(data, studies, treatments, model)
}
...
#' @param studies Study identifier column name
#' @param treatments Treatment identifier column name
#' @param model Model type: "random", "fixed", or "component"
validate_network_structure <- function(data, studies, treatments, model = "random") {
  ...
  # Skip connectivity and sparse comparison checks for Component NMA
  # CNMA can handle disconnected networks if components bridge the gaps
  if (model == "component") {
...
extract_nma_results <- function(nma_fit, network_data, reference) {
  ...
  # The mods parameter in rma.mv makes beta a vector of effects
...
print.cbamm_network <- function(x, ...) {
  ...
  if (x$model == "component") {
    cat("Component Effects:\n")
    print(x$results$component_effects, row.names = FALSE)
    cat("\nTreatment Reconstruction:\n")
    print(x$results$treatment_reconstruction, row.names = FALSE)
  } else {
...
plot_forest_network <- function(x, ...) {
  ...
  if (x$model == "component") {
    data <- x$results$component_effects
    mapping <- ggplot2::aes(x = estimate, y = component)
    title <- "Component NMA Effects"
  } else {
    data <- x$results$treatment_effects
    data <- data[data$treatment != x$reference, ]  # Remove reference
    mapping <- ggplot2::aes(x = estimate, y = treatment)
    title <- "Network Meta-Analysis Forest Plot"
  }
  
  p <- ggplot2::ggplot(data, mapping) +
    ggplot2::geom_point(size = 3) +
    ggplot2::geom_errorbarh(ggplot2::aes(xmin = ci_lb %||% ci_lower, xmax = ci_ub %||% ci_upper), height = 0.2) +
    ggplot2::geom_vline(xintercept = 0, linetype = "dashed", color = "red") +
    ggplot2::labs(
      title = title,
      subtitle = if(x$model != "component") paste("Reference treatment:", x$reference) else NULL,
      x = "Effect Size",
      y = if(x$model == "component") "Component" else "Treatment"
    ) +
...
#' @param network_data Prepared network data
#' @param reference Reference treatment
#' @return Formatted CNMA results
#' @keywords internal
run_component_nma <- function(network_data, reference) {
  
  # 1. Identify components (e.g., "A+B" -> "A", "B")
  # We assume components are separated by "+" or " + "
  treatments <- unique(network_data$treatment)
  all_comps <- unique(unlist(strsplit(treatments, "\\s*\\+\\s*")))
  
  # 2. Create Component Matrix
  comp_mat <- create_component_matrix(treatments, all_comps)
  
  # 3. Prepare data for model
  # Convert long format to contrast format
  contrast_data <- create_contrast_data(network_data, reference)
  V <- create_vcov_matrix(contrast_data)
  
  # Map contrasts to component differences
  # Contrast(T1 vs Ref) = sum(Components in T1) - sum(Components in Ref)
  
  ref_comps <- if (reference %in% rownames(comp_mat)) comp_mat[reference, ] else rep(0, ncol(comp_mat))
  
  X_comp <- matrix(0, nrow = nrow(contrast_data), ncol = ncol(comp_mat))
  colnames(X_comp) <- colnames(comp_mat)
  
  for (i in 1:nrow(contrast_data)) {
    trt <- contrast_data$treatment[i]
    if (trt %in% rownames(comp_mat)) {
      X_comp[i, ] <- comp_mat[trt, ] - ref_comps
    } else {
      # Handle "T1 vs T2" type contrasts
      parts <- strsplit(trt, " vs ")[[1]]
      if (length(parts) == 2) {
        X_comp[i, ] <- comp_mat[parts[1], ] - comp_mat[parts[2], ]
      }
    }
  }
  
  # 4. Fit additive model using metafor
  if (requireNamespace("metafor", quietly = TRUE)) {
    fit <- try(metafor::rma.mv(yi = contrast_data$effect, 
                              V = V, 
                              mods = X_comp, 
                              intercept = FALSE,
                              method = "REML"), silent = TRUE)
    
    if (inherits(fit, "try-error")) stop("Component NMA failed: ", as.character(fit))
    
    # 5. Extract Component Effects
    comp_effects <- data.frame(
      component = colnames(X_comp),
    ...
    # 6. Reconstruct Treatment Effects from Components
    recon_estimates <- comp_mat %*% fit$beta
    
    # Pack results in a way that rankings can still work
    res_list <- list(
      component_effects = comp_effects,
      treatment_reconstruction = data.frame(
        treatment = rownames(comp_mat),
        reconstructed_estimate = as.numeric(recon_estimates)
      ),
      # Create a treatment_effects df for S3 methods compatibility
      treatment_effects = data.frame(
        treatment = rownames(comp_mat),
        estimate = as.numeric(recon_estimates),
        stringsAsFactors = FALSE
      ),
      model_object = fit,
      type = "component"
    ...
    stop("metafor package required for Component NMA")
  }
}
...
#' @param components Vector of unique component names
#' @return Binary matrix (Treatments x Components)
#' @keywords internal
create_component_matrix <- function(treatments, components) {
  mat <- matrix(0, nrow = length(treatments), ncol = length(components))
  rownames(mat) <- treatments
  colnames(mat) <- components
  
  for (trt in treatments) {
    comps_in_trt <- strsplit(trt, "\\s*\\+\\s*")[[1]]
    mat[trt, comps_in_trt] <- 1
  }
