...
#' Check Bayesian Convergence and Auto-Extend
#' @param fit Model fit object
#' @param type "brms" or "jags"
#' @param threshold R-hat threshold (default 1.1)
#' @return List with convergence status and diagnostics
#' @keywords internal
check_bayesian_convergence <- function(fit, type = "brms", threshold = 1.1) {
  
  if (type == "brms") {
    rhats <- brms::rhat(fit)
    converged <- all(rhats < threshold, na.rm = TRUE)
    ...
    if (!check_package("coda", quietly = TRUE)) {
      return(list(converged = TRUE, message = "coda not available for check"))
    }
    
    gelman <- try(coda::gelman.diag(fit, multivariate = FALSE), silent = TRUE)
    if (inherits(gelman, "try-error")) {
      return(list(converged = FALSE, error = "Gelman diagnostic failed"))
    }
    
    rhats <- gelman$psrf[, "Point est."]
    converged <- all(rhats < threshold, na.rm = TRUE)
...
compute_bayesian_stacking <- function(bayesian_results, data, config) {
  ...
  # 1. Try to use LOO for stacking if available (Pseudo-BMA)
  # This currently only works if multiple models have LOO results
  loo_list <- lapply(bayesian_results, function(x) x$loo)
  valid_loo <- !sapply(loo_list, is.null)
  
  if (sum(valid_loo) >= 2 && check_package("loo", quietly = TRUE)) {
    try({
      loo_compare <- loo::loo_model_weights(loo_list[valid_loo], method = "stacking")
      weights[valid_loo] <- as.numeric(loo_compare)
    }, silent = TRUE)
    
  } else {
    # 2. Informative fallback: Variance-weighted averaging
    # Higher precision (lower variance) gets more weight
    
    precisions <- rep(NA_real_, n_methods)
    names(precisions) <- methods
    
    for (m in methods) {
      if (m == "brms" && !is.null(bayesian_results[[m]]$summary)) {
        precisions[m] <- 1 / (as.numeric(bayesian_results[[m]]$summary["b_Intercept", "Est.Error"])^2)
      } else if (m == "jags" && !is.null(bayesian_results[[m]]$mu_summary)) {
        precisions[m] <- 1 / (as.numeric(bayesian_results[[m]]$mu_summary["sd"])^2)
      }
    }
    
    if (all(!is.na(precisions)) && sum(precisions) > 0) {
