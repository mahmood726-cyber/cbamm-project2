...
print.cbamm_results <- function(x, ...) {
  ...
  if (!is.null(x$cross_design_results$combined)) {
    cd <- x$cross_design_results$combined
    cat(sprintf("Cross-design effect: %.3f (95%% CI: %.3f-%.3f)\n",
               cd$effect, cd$ci_lower, cd$ci_upper))
  }
...
summary.cbamm_results <- function(object, ...) {
  ...
  if (!is.null(object$cross_design_results$combined)) {
    fit <- object$cross_design_results$combined
    rows[[length(rows) + 1L]] <- data.frame(
      Method = "Cross-design combined",
      Estimate = fit$effect,
      SE = fit$se,
      I_squared = fit$I2,
      Studies = fit$k
  ...
  summary_df <- if (length(rows) == 0) data.frame() else do.call(rbind, rows)
