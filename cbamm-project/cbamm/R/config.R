...
cbamm_config <- function(
  ...
  # Merge methods
  methods <- merge_methods(default_methods, methods)
  ...
  output <- modifyList(default_output, output)
  
  # Default transportability
  default_transport <- list(
    method = "entropy",
    target_population = NULL,
    covariates = NULL
  )
  transportability <- modifyList(default_transport, transportability)
  
  # Default bayesian
  default_bayesian <- list(
    method = "brms",
    chains = 4,
    iter = 2000,
    warmup = 1000,
  ...
  bayesian <- modifyList(default_bayesian, bayesian)
  
  # Default sensitivity
  default_sensitivity <- list(
    egger_test = TRUE,
    begg_test = TRUE,
    trim_fill = TRUE,
  ...
  sensitivity <- modifyList(default_sensitivity, sensitivity)
