...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Multiverse", tabName = "multiverse", icon = icon("layer-group")),
  ...
  dashboardBody(
    tabItems(
        ...
        fluidRow(
          box(
            ...
            selectInput("analysis_type", "Analysis Type:",
                       choices = c("Standard (REML/HKSJ)" = "standard",
                                 "IPD Meta-Analysis" = "ipd",
                                 "Network Meta-Analysis" = "nma")),
            
            selectInput("performance_mode", "Performance Mode:",
                       choices = c("Fast (Standard)" = "fast",
                                 "Comprehensive (Full)" = "comprehensive")),
            
            checkboxGroupInput("methods", "Enable Methods:",
                              choices = c("Transport Weighting" = "transport",
                                        "Bayesian Stacking" = "bayesian",
                                        "Conflict Detection" = "conflict",
                                        "Multiverse Analysis" = "multiverse"),
                              selected = c("transport", "conflict")),
            
            actionButton("run_analysis", "Run Analysis", 
                        class = "btn-success btn-lg", icon = icon("play")),
            
            hr(),
            downloadButton("download_report", "Generate TruthCert Report")
          ),
          
          box(
            title = "Console Output",
            width = 8,
            verbatimTextOutput("analysis_output", placeholder = TRUE)
          )
        )
      ),
      ...
      tabItem(
        tabName = "multiverse",
        h2("Multiverse Analysis"),
        fluidRow(
          box(
            title = "Methodological Sensitivity",
            width = 12,
            plotlyOutput("multiverse_plot", height = "600px"),
            hr(),
            DT::dataTableOutput("multiverse_table")
          )
        )
      ),
...
server <- function(input, output, session) {
  ...
  # Multiverse analysis output
  output$multiverse_plot <- renderPlotly({
    req(values$results$multiverse)
    mv <- values$results$multiverse
    
    plot_ly(mv, x = ~estimate, y = ~estimator, color = ~subset, 
            type = "scatter", mode = "markers",
            error_x = list(array = ~se * 1.96)) %>%
      layout(title = "Effect Estimates Across Methodological Multiverse",
             xaxis = list(title = "Pooled Effect (Hazard Ratio scale)"),
             yaxis = list(title = "τ² Estimator"))
  })
  
  output$multiverse_table <- DT::renderDataTable({
    req(values$results$multiverse)
    DT::datatable(values$results$multiverse, options = list(pageLength = 5))
  })
  
  # Report downloading
  output$download_report <- downloadHandler(
    filename = function() {
      paste0("cbamm_report_", format(Sys.time(), "%Y%m%d_%H%M"), ".html")
    },
    content = function(file) {
      req(values$results)
      create_cbamm_report(values$results, filename = file)
    }
  )
