...
run_adaptive_advisor <- function(data, pooled_results, config) {
  ...
  if (config$output$verbose) {
    ...
    message("Inference: ", if (config$methods$hksj) "HKSJ (Knapp–Hartung)" else "Wald z")
...
detect_study_conflicts <- function(data, threshold = 0.15, k_candidates = 2:4) {
  ...
  pick_k <- 2L
  if (check_package("cluster", quietly = TRUE)) {
    best_k <- 2
    best_s <- -Inf
    
    for (K in intersect(k_candidates, 2:min(10, floor(nrow(data)/2)))) {
  ...
  w_all <- if ("analysis_weights_grade" %in% names(df)) {
    with(df, (1 / (se^2)) * dplyr::coalesce(analysis_weights_grade, analysis_weights, 1))
  ...
  comp <- df %>%
    ...
    dplyr::summarise(
      n = dplyr::n(),
      mean_logHR = stats::weighted.mean(yi, w, na.rm = TRUE),
      HR = exp(mean_logHR),
      mean_year = if ("year" %in% names(.)) mean(year, na.rm = TRUE) else NA_real_,
      prop_RCT = if ("study_type" %in% names(.)) mean(study_type == "RCT", na.rm = TRUE) else NA_real_,
      prop_OBS = if ("study_type" %in% names(.)) mean(study_type == "OBS", na.rm = TRUE) else NA_real_,
      prop_MR  = if ("study_type" %in% names(.)) mean(study_type == "MR",  na.rm = TRUE) else NA_real_,
      mean_grade = if ("grade" %in% names(.)) mean(as.numeric(grade), na.rm = TRUE) else NA_real_,
      .groups = "drop"
    )
  ...
  if (pick_k == 2L) {
    ...
    predictors <- c()
    if ("study_type" %in% names(df)) predictors <- c(predictors, "study_type")
    if ("year" %in% names(df)) predictors <- c(predictors, "scale(year)")
    if ("grade" %in% names(df)) predictors <- c(predictors, "as.numeric(grade)")
    
    if (length(predictors) > 0) {
      formula_str <- paste("cl_bin ~", paste(predictors, collapse = " + "))
      mod <- try(glm(as.formula(formula_str), data = df, family = binomial()), silent = TRUE)
